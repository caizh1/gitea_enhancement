...
def run(name, port, subnet, image, replacement=False):
    ...
    original = (WORK / 'compose.yml').read_text()
    compose.write_text(original.replace('gitea-governance-navigation:review-20260916', image).replace('10.251.240.0/28', subnet))
    env = folder / '.env'
    if not env.exists():
    ...
    before = command(['docker','exec',container,'gitea','--version']).strip()
    if replacement:
        compose.write_text(compose.read_text().replace(image,'gitea-governance-navigation:review-20260916'))
        command(prefix+['up','-d'])
        wait(port)
...
if __name__ == '__main__':
    try:
        run('demo-final7',3516,'10.251.246.0/28',os.environ.get('OLD_DEMO_IMAGE','sha256:b4cf03b8d552850303d49c32b1dd452c11c6df313966c6a1f43b99833683d87d'),True)
        run('fresh-final7',3517,'10.251.247.0/28','gitea-governance-navigation:review-20260916')
    finally:
        (WORK/'installation-results.json').write_text(json.dumps(RESULTS,ensure_ascii=False,indent=2))
